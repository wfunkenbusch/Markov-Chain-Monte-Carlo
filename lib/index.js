...
function main(options) {
  ...
  */
  ...
  // Counts number of edges connected to source node (non-zero elements in first row/column of adjacency matrix) and total number of edges in graph
  var sourceEdges = 0;
  var totalEdges = 0;
  for (var l = 0; l < Fun.AList[0][0].length; l++) {
    if (Fun.AList[0][0][l] !== 0) {
      sourceEdges += 1;
    }

    for (var m = 0; m < Fun.AList[0][0].length; m++) {
      if (m > l && Fun.AList[0][l][m] !== 0) {
        totalEdges += 1;
      }
    }
  }
  ...
  for (var i = 0; i < N - 1; i++) {
    ...
    // Counts number of edges connected to source node (non-zero elements in first row of adjacency matrix) and total number of edges in graph
    for (var j = 0; j < Fun.AList[0][0].length; j++) {
      if (Fun.AList[Fun.AList.length - 1][0][j] !== 0) {
        sourceEdges += 1;
      }

      for (var n = 0; n < Fun.AList[0][0].length; n++) {
        if (n > j && Fun.AList[Fun.AList.length - 1][j][n]) {
          totalEdges += 1;
        }
      }
    }
  ...
  console.log('Expected number of edges connected to source node:', sourceEdges / N);
  console.log('Expected total number of edges', totalEdges / N);
  console.log(
...
module.exports = { Graph, convert, main };
