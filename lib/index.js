...
function simulSort(A, w) {
  /*
  Sorts a array with respect to a array of weights. Sorted highest to lowest.
  ...
  */
  ...
  var ASort = new Array(A.length);
  var wSort = new Array(w.length);

  // Finds proper position of each element
  for (var i = 0; i < A.length; i++) {
    ...
    // Move up one space if there is already a value stored
    pos += -1;
    do {
      pos += 1;
    } while (wSort[pos] === w[i]);
...
function main(options) {
  ...
  */
  ...
  for (var l = 0; l < Fun.G.nodes().length; l++) {
  ...
  // Prints top 1% of adjacency matrices and the number of times they appeared.
  // Prioritizes graphs which appeared first.
  var [AListSort, AListCountSort] = simulSort(Fun.AList, Fun.AListCount);
  var bestGraphs = [];
  var bestGraphsCount = [];
  var nBestGraphs = Math.floor(Fun.AList.length / 100 + 1);
  for (var m = 0; m < nBestGraphs; m++) {
    bestGraphs.push(AListSort[m]);
    bestGraphsCount.push(AListCountSort[m]);
    console.log(bestGraphs[m], bestGraphsCount[m]);
  }
...
module.exports = { Graph, convert, sourceNodeEdges, edges, simulSort, main };
